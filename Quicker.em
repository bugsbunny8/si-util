/****************************************************
 * Source Insight Quicker Command Macros
 *
 ****************************************************/

/* main AutoExpand Source Insight Macro. */
macro AutoExpand()
{
    //配置信息
    ...
    else if (wordinfo.szWord == "help" || wordinfo.szWord == "h")
    {
        DelBufLine(hbuf, ln)
        QuickerHelp(hbuf, ln)
        return
    ...
    if(language == 1)
    {
        ExpandProcEN(szUserName, wordinfo,szLine,szLine1,nVer,ln,sel)
    }
    else
    {
        ExpandProcCN(szUserName, wordinfo,szLine,szLine1,nVer,ln,sel)
    }
}

/* help macro */
macro QuickerHelp(hbuf, ln)
{
	Line = ln;

    InsBufLine(hbuf, Line++,   "/*****************Quicker Macro Help***************"); 
    InsBufLine(hbuf, Line++,   " \/\*\t\t\t\t: expand comment ")
    InsBufLine(hbuf, Line++,   " \{ \t\t\t\t: expand {} ")
    InsBufLine(hbuf, Line++,   " while \t\t\t: expand while ")
    InsBufLine(hbuf, Line++,   " if \t\t\t: expand if ")
    InsBufLine(hbuf, Line++,   " for \t\t\t: expand for with local variables")
    InsBufLine(hbuf, Line++,   " fo \t\t\t: expand for ")
    InsBufLine(hbuf, Line++,   " do \t\t\t: expand do while statement ")
    InsBufLine(hbuf, Line++,   " #ifd \t\t\t: expand #ifdefine statement")
    InsBufLine(hbuf, Line++,   " #ifn \t\t\t: expand #ifndef statement")
    InsBufLine(hbuf, Line++,   " #if \t\t\t: expand #if statement ")
    InsBufLine(hbuf, Line++,   " cpp \t\t\t: expand cpp statement prototype")
    InsBufLine(hbuf, Line++,   " switch \t\t: expand switch statement")
    InsBufLine(hbuf, Line++,   " case \t\t\t: expand cpp statement prototype")
    InsBufLine(hbuf, Line++,   " struct \t\t: expand struct statement prototype")
    InsBufLine(hbuf, Line++,   " enum \t\t\t: expand enum statement prototype")
    InsBufLine(hbuf, Line++,   "")
    InsBufLine(hbuf, Line++,   " config/co \t\t: config user information")
    InsBufLine(hbuf, Line++,   " file/fi \t\t: generate file header")
    InsBufLine(hbuf, Line++,   " func/fu \t\t: generate function header")
    InsBufLine(hbuf, Line++,   " hi \t\t\t: generate increment history")
    InsBufLine(hbuf, Line++,   " hd \t\t\t: generate func header")
    InsBufLine(hbuf, Line++,   "*******************************************************/")
}
...
{   
	...
	/*Group Name*/
    szGroup = getreg(GROUP_NAME)
	/*Company Name*/
    szCompany = getreg(COMPANY_NAME)
	/*Current File Name*/
    szFile = GetFileName(GetBufName (hbuf))
	/*Get Current Time as create time */
    SysTime = GetSysTime(1)
    szYear=SysTime.Year
    ...
    InsBufLine(hbuf, ln + 1,  " * Copyright 2016-2019 by @szGroup@, @szCompany@")
    InsBufLine(hbuf, ln + 2,  " * All right reserved. See COPYRIGHT for detailed information.")
    InsBufLine(hbuf, ln + 3,  " *")
    InsBufLine(hbuf, ln + 4,  " * \@file\t\t@szFile@")
    InsBufLine(hbuf, ln + 5,  " * \@brief\t\t@szContent@")
    InsBufLine(hbuf, ln + 6,  " * \@author\t\t@szName@")
    InsBufLine(hbuf, ln + 7,  " * \@date\t\t@szYear@/@szMonth@/@szDay@")
    InsBufLine(hbuf, ln + 8,  " * \@functions")

    nlnDesc = ln
    
    //插入函数列表
    ln = InsertFileList(hbuf,hnewbuf,ln + 9) - 9
    closebuf(hnewbuf)

	// insert history
    InsBufLine(hbuf, ln + 10, " * \@history\t1. @szName@ Created File in @szYear@/@szMonth@/@szDay@")
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  " * 版权所有 (C), 2016-2019, ")
    InsBufLine(hbuf, ln + 2,  " * 保留所有授权，详细参见\"版权说明\"文件")
    InsBufLine(hbuf, ln + 3,  " *")
    InsBufLine(hbuf, ln + 4,  " * 文件名\t\t: @szFileName@")
    InsBufLine(hbuf, ln + 5,  " * 产品名\t\t: ")
    InsBufLine(hbuf, ln + 6,  " * 模块名\t\t: ")
    InsBufLine(hbuf, ln + 7,  " * 作者\t\t: @szName@")
    SysTime = GetSysTime(1)
    szTime = SysTime.Date
    ...
    InsBufLine(hbuf, ln + 11, " * 修改历史\t\t: 1. @szTime@ @szName@ 创建文件")
    InsBufLine(hbuf, ln + 12, " *")
    InsBufLine(hbuf, ln + 13, " ******************************************************************************/")
    ...
    //如果没有输入功能描述的话提示输入
    szContent = Ask("请输入文件的功能描述：")
    SetBufIns(hbuf, nlnDesc + 14, 0)

    DelBufLine(hbuf,nFuncDesc)    
    //自动排列显示功能描述
    CommentContent(hbuf, nFuncDesc, " * 功能描述\t\t: ", szContent, 0)
}
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if( strlen(szFunc)>0 )
    {
        InsBufLine(hbuf, ln+1, " * 函数名\t\t: @szFunc@")
    }
    else
    {
    ...
    InsBufLine(hbuf, ln+4, " * 返回值\t\t: @szRet@")
    InsBufLine(hbuf, ln+5, " * 注意事项\t\t: ")
    InsbufLIne(hbuf, ln+6, " *");
    InsBufLine(hbuf, ln+7, " * 修改历史\t\t:")
    SysTime = GetSysTime(1);
    szTime = SysTime.Date

    InsBufLine(hbuf, ln+8,  "  1.@szTime@ @szMyName@ 新生成函数")
...
macro FuncHeadCommentEN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    InsBufLine(hbuf, ln+6, " * \@see\t\t")
    InsbufLIne(hbuf, ln+7, " * \@note\t\t");
    ...
    InsBufLine(hbuf, ln + 9, " * \@history")
    InsBufLine(hbuf, ln + 10, "\t1. @sz1@/@sz2@/@sz3@, @szMyName@ Create Function")
    InsBufLine(hbuf, ln + 11, " *")
    InsBufLine(hbuf, ln + 12, " *")    
    InsBufLine(hbuf, ln + 13, " *****************************************************************************/")
    if ((newFunc == 1) && (strlen(szFunc)>0))
...
macro  InsertHistoryContentCN(hbuf,ln,iHostoryCount)
{
    ...
    {
       InsBufLine(hbuf, ln + 2, "    作    者   : @szUserName@")
    }
    else
    {
...
macro ConfigureSystem()
{
    ...
    szUserName = ASK("Please input User Name");
    if(szUserName == "#")
    {
       SetReg ("USER_NAME", "")
    }
    ...
    {
       SetReg ("USER_NAME", szUserName)
    }
    
    /* email configuration */
    szUserEmail = ASK("Please input email(default enter #)");
    if(szUserEmail == "#")
    {
       SetReg ("USER_EMAIL", szUserName #"\@hygon.cn")
    }
    else
    {
       SetReg ("USER_EMAIL", szUserEmail)
    }

    /* Group configuration */
    szGroupName = ASK("Please input Group Name");
    if(szGroupName == "#")
    {
       SetReg ("GROUP_NAME", "")
    }
    ...
    {
       SetReg ("GROUP_NAME", szGroupName)
    }

    /* Company configuration */
    szCompanyName = ASK("Please input your Company Name");
    if(szCompanyName == "#")
    {
       SetReg ("COMPANY_NAME", "")
    }
    ...
    {
       SetReg ("COMPANY_NAME", szCompanyName)
    }
    
}

/* get User Information, if not exist in system, ask and save in register */
macro GetUserInfoSave(szREGNAME,      szHintInfo)
{
    /*取得用户信息*/
    szUserInfo = getreg(szREGNAME)
    if(strlen( szUserInfo ) == 0)
    {
        szUserInfo = Ask(szHintInfo)
        /* 保存于注册表位置为
         * HKEY_CURRENT_USER\Software\Source Dynamics\Source Insight\4.0
         * 下面的注册表项
         * 注册表项名为传入参数szREGNAME  */
        setreg(szREGNAME, szUserInfo)
    }
    
    return szUserInfo
}

macro GetUserNameSave()
{
	return GetUserInfoSave(USER_NAME, "Enter your name:")
}

macro GetUserEmailSave()
{
	return GetUserInfoSave(USER_EMAIL, "Enter your email:")
}

/* return xxxx <xxx@xxx.com> format  user information */
macro GetUserNameEmailSave()
{
	szUserName = GetUserNameSave();
	szUserEmail = GetUserEmailSave();
	return szUserName # "<" # szUserEmail # ">"
}
